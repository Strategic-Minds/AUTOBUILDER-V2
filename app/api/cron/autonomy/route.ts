...
import { dispatchControlledAutonomy } from '@/lib/autonomy/build-dispatcher';
...
export async function GET(req: NextRequest) {
  ...
  const controlled = await dispatchControlledAutonomy(`${workerId}-builds`, 4);
  const generic: Array<Record<string, unknown>> = [];

  for (let index = 0; index < 4; index += 1) {
    const job = await claimNextAutonomyJob(`${workerId}-generic`);
    if (!job) break;
    ...
    try {
      ...
      if (result.ok) {
        ...
        generic.push({ job_id: job.job_id || job.id, state: 'completed', provider: result.provider });
      } else {
        ...
        generic.push({ job_id: job.job_id || job.id, state: requeue ? 'requeued' : 'failed', provider: result.provider });
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      await failAutonomyJob(job, message, true);
      generic.push({ job_id: job.job_id || job.id, state: 'requeued', error: message });
    }
  }

  return NextResponse.json({
    ok: true,
    state: controlled.claimed || generic.length ? 'DISPATCHED' : 'IDLE',
    worker_id: workerId,
    controlled,
    generic,
