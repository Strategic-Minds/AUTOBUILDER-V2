...
begin
  ...
  if exists (
    ...
    where table_schema = 'public' and table_name = 'factory_repair_jobs'
      and column_name in ('project_id', 'finding_id', 'repair_type', 'recipe')
  ) then
  ...
  foreach table_name in array array[
    ...
    if exists (
      ...
      raise exception 'RLS remains enabled after rollback: %', table_name;
    end if;

    if not has_table_privilege('anon', 'public.' || table_name, 'SELECT,INSERT,UPDATE,DELETE') then
      raise exception 'anon pre-migration privileges not restored: %', table_name;
    end if;

    if not has_table_privilege('authenticated', 'public.' || table_name, 'SELECT,INSERT,UPDATE,DELETE') then
      raise exception 'authenticated pre-migration privileges not restored: %', table_name;
    end if;
  end loop;
end
$$;
