...
function config() {
  ...
  if (!url || !key) throw new Error('Xtreme AI Builder database is not configured')
  return { url, key }
}
...
async function db<T>(path: string, method = 'GET', body?: unknown, prefer = 'return=representation'): Promise<T> {
  ...
  if (!response.ok) throw new Error(`Xtreme AI Builder database ${method} failed ${response.status}: ${raw.slice(0, 500)}`)
  return parsed as T
}

export async function createRun(cycleId: string) {
  const rows = await db<Array<{ id: string }>>('xab_validation_runs', 'POST', {
  ...
  if (!rows[0]) throw new Error('Validation run creation returned no row')
  return rows[0]
}

export async function saveBrowserJob(runId: string, externalJobId: string, viewport: string, request: JsonRecord) {
  const rows = await db<Array<{ id: string }>>('xab_validation_browser_jobs', 'POST', {
    run_id: runId,
...
export async function finishBrowserJob(externalJobId: string, passed: boolean, result: JsonRecord, error?: string) {
  return db(`xab_validation_browser_jobs?external_job_id=eq.${encodeURIComponent(externalJobId)}`, 'PATCH', {
...
export async function finishRun(runId: string, input: { score: number; releaseGate: string; findings: unknown[]; browserEvidence: JsonRecord; error?: string }) {
  return db(`xab_validation_runs?id=eq.${encodeURIComponent(runId)}`, 'PATCH', {
...
export async function saveReceipt(runId: string, kind: string, passed: boolean, details: JsonRecord) {
  const rows = await db<Array<{ id: string }>>('xab_validation_receipts', 'POST', { run_id: runId, kind, passed, details })
  return rows[0]
}

export async function latestRun() {
  const rows = await db<JsonRecord[]>(`xab_validation_runs?mission_id=eq.${MISSION_ID}&order=created_at.desc&limit=1`)
  return rows[0] || null
}
